<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
                <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200 leading-tight">Add Question</h2>
                <p class="text-xs text-gray-400 mt-0.5 truncate">{{ $quiz->title }}
                    · {{ $quiz->questions()->count() }} {{ Str::plural('question', $quiz->questions()->count()) }}</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if (session('success'))
                <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
                    class="mb-4 p-3 bg-green-50 border border-green-200 text-green-800 rounded-lg text-sm flex items-center gap-2">
            ...
            <form method="POST" action="{{ route('quiz.questions.store', $quiz) }}"
                x-data="questionForm('{{ old('question_type', 'single') }}')"
                @submit.prevent="submitWith(pendingAction)">
                @csrf
                ...
                <div class="space-y-5">
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
                        ...
                        <div x-show="langTab === 'en'">
                            <textarea name="question_text" rows="4" required
                                ...
                                class="w-full text-sm rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-primary/30 focus:border-primary resize-none">{{ old('question_text') }}</textarea>
                        </div>
                        ...
                        <div x-show="langTab === 'hi'" x-cloak>
                            <textarea name="question_text_hi" rows="4"
                                ...
                                dir="auto"
                                class="w-full text-sm rounded-lg border-orange-200 dark:border-orange-700/60 dark:bg-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-orange-300/40 focus:border-orange-400 resize-none">{{ old('question_text_hi') }}</textarea>
                    ...
                    <div x-show="type === 'text'"
                        ...
                        <textarea name="correct_text_answer" rows="3"
                            ...
                            class="w-full text-sm rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-primary/30 focus:border-primary resize-none">{{ old('correct_text_answer') }}</textarea>
                    </div>
                    ...
                    <div
                        ...
                        <div>
                            ...
                            <input type="number" name="marks" step="0.5" min="0"
                                value="{{ old('marks') }}" placeholder="{{ $quiz->marks_per_question }}"
                        ...
                        <div x-data="{ showNotes: {{ old('notes') ? 'true' : 'false' }} }">
                            ...
                            <div x-show="showNotes" x-cloak class="mt-2">
                                <textarea name="notes" rows="3"
                                    ...
                                    class="w-full text-sm rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-primary/30 focus:border-primary resize-none">{{ old('notes') }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 pb-6">
                        ...
                        <button type="button" @click="submitWith('save_add')"
                            class="flex-1 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 text-sm font-semibold rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition flex items-center justify-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 4v16m8-8H4" />
                            </svg>
