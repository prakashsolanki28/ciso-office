    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5">
                <dl class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                    <div>
                        ...
                        <dd
                            class="mt-1 font-medium text-gray-800 dark:text-gray-200 {{ $session->name ? '' : 'italic text-gray-400 font-normal' }}">
                            {{ $session->display_name }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs text-gray-400 uppercase tracking-wider">Messages</dt>
                        <dd class="mt-1 font-medium text-gray-800 dark:text-gray-200">{{ $session->messages->count() }}
                        </dd>
                    </div>
                    <div>
                        ...
                        <dd class="mt-1 font-medium text-gray-800 dark:text-gray-200">
                            {{ $session->created_at->format('M d, Y H:i') }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs text-gray-400 uppercase tracking-wider">Last active</dt>
                        <dd class="mt-1 font-medium text-gray-800 dark:text-gray-200">
                            {{ optional($session->last_active_at)->diffForHumans() ?? '—' }}</dd>
                    </div>
                    ...
                    <div class="col-span-2">
                        ...
                        <dd class="mt-1 text-xs text-gray-500 dark:text-gray-400 break-words">
                            {{ $session->user_agent ?? '—' }}</dd>
                    </div>
                </dl>
            </div>
                    ...
                    <div class="p-5 space-y-4 bg-[#f8f9fb] dark:bg-gray-900/30">
                        @foreach ($session->messages as $message)
                            ...
                            <div class="flex gap-2.5 items-end {{ $isUser ? 'flex-row-reverse' : '' }}">
                                <div
                                    class="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 {{ $isUser ? 'bg-alert-amber/20' : 'bg-primary/10' }}">
                                    <i data-lucide="{{ $isUser ? 'user' : 'bot' }}"
                                        class="text-sm {{ $isUser ? 'text-alert-amber' : 'text-primary' }}"></i>
                                </div>
                                <div class="max-w-[78%]">
                                    <div
                                        class="rounded-2xl px-4 py-2.5 text-sm shadow-sm whitespace-pre-wrap break-words {{ $isUser ? 'bg-[#040f24] text-white rounded-br-sm' : 'bg-white border border-gray-200 text-on-surface rounded-bl-sm' }}">
                                        {{ $message->content }}</div>
                                    <div class="mt-1 text-[10px] text-gray-400 {{ $isUser ? 'text-right' : '' }}">
                                        {{ $message->created_at->format('M d, H:i') }}</div>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="flex justify-end">
                <form method="POST" action="{{ route('manjulika.sessions.destroy', $session) }}" x-data
