<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('casestudy.index')" :active="request()->routeIs('casestudy.*')">
                        {{ __('Case Studies') }}
                    </x-nav-link>
                    <x-nav-link :href="route('sop.index')" :active="request()->routeIs('sop.*')">
                        {{ __('SOPs') }}
                    </x-nav-link>
                    <x-nav-link :href="route('awareness')" :active="request()->routeIs('awareness')">
                        {{ __('Awareness') }}
                    </x-nav-link>
                    @else
                    <x-nav-link :href="route('quiz.take.index')" :active="request()->routeIs('quiz.take.*')">
                        {{ __('My Quizzes') }}
                    </x-nav-link>
                    @endif
                </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <x-responsive-nav-link :href="route('casestudy.index')" :active="request()->routeIs('casestudy.*')">
                {{ __('Case Studies') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('sop.index')" :active="request()->routeIs('sop.*')">
                {{ __('SOPs') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('awareness')" :active="request()->routeIs('awareness')">
                {{ __('Awareness') }}
            </x-responsive-nav-link>
            @else
            <x-responsive-nav-link :href="route('quiz.take.index')" :active="request()->routeIs('quiz.take.*')">
                {{ __('My Quizzes') }}
            </x-responsive-nav-link>
            @endif
        </div>
